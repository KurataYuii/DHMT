...
float viewerDerection[3] = { 0.0,0.0,0.0 };
float viewerUp[3] = { 0.0,0.0,0.0 };
float nagigationRotion[3] = { 0.0,0.0,0.0 };
...
void displayFunc(void) {
	...
	glRotatef(nagigationRotion[0], 1.0f, 0.0f, 0.0f);
	glRotatef(nagigationRotion[1], 0.0f, 1.0f, 0.0f);
...
void initGlut(int argc, char **argv) {
	...
	nWindowID - glutCreateWindow("Mệt");
...
void mouseMotionFunc(int x, int y) {
	...
	if (leftMouseButtonActive) {
		nagigationRotion[0] += ((mousePressedY - y)*180.0f) / 200.0f;
		nagigationRotion[1] += ((mousePressedX - x)*180.0f) / 200.0f;
		mousePressedY = y;
		mousePressedX = x;
	}
	else if (middleMouseButtonActive)
	{
		...
		if (!lastXOffset == 0.0) {
			...
			viewerDerection[0] -= (xOffset - lastXOffset) / 8.0;
		}
		lastXOffset = xOffset;
		yOffset = (mousePressedY + y);
		if (!lastYOffset == 0.0) {
			viewerPosition[1] += (yOffset - lastYOffset) / 8.0;
			viewerDerection[1] += (yOffset - lastYOffset) / 8.0;
		}
		lastYOffset = yOffset;
	}
	else if(rightMouseButtonActive)
	{
		...
		if (!lastZOffset == 0.0) {
			...
			viewerDerection[2] -= (zOffset - lastZOffset) / 8.0;
		}
		lastZOffset = zOffset;
	}
}
